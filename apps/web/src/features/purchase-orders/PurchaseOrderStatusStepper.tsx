...
const STEPS: { key: string; label: string; statuses: PurchaseOrderStatus[] }[] = [
  { key: 'draft', label: 'Draft', statuses: ['draft'] },
  { key: 'submitted', label: 'Submitted', statuses: ['submitted'] },
  { key: 'approved', label: 'Approved', statuses: ['approved'] },
  { key: 'received', label: 'Received', statuses: ['partially_received', 'fully_received'] },
  { key: 'closed', label: 'Closed', statuses: ['closed'] },
];

const TERMINAL_LABEL: Partial<Record<PurchaseOrderStatus, string>> = {
  rejected: 'Rejected',
  cancelled: 'Cancelled',
};

/** Index of the furthest step reached before a terminal rejection/cancellation. */
function stepIndexForStatus(status: PurchaseOrderStatus): number {
...
export function PurchaseOrderStatusStepper({ status }: { status: PurchaseOrderStatus }) {
