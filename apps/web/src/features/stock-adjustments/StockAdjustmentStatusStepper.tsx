...
const STEPS: { key: StockAdjustmentStatus; label: string }[] = [
...
/** `reversed` is a stamp on the original posted adjustment, shown as a banner (mirrors GoodsReceiptStatusStepper). */
export function StockAdjustmentStatusStepper({
  status,
  isReversed,
}: {
    ...
    <div className="space-y-3">
      ...
      {isReversed && (
        <div className="flex items-center gap-2 rounded-md border border-dashed border-muted-foreground/40 px-3 py-2 text-xs text-muted-foreground">
          ...
          This adjustment has been reversed by a linked reversal adjustment.
        </div>
      )}
    </div>
  );
